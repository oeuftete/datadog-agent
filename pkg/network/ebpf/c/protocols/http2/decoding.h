...
#pragma unroll (HTTP2_MAX_FRAMES_TO_FILTER)
    for (__u32 iteration = 0; iteration < HTTP2_MAX_FRAMES_TO_FILTER; ++iteration) {
        ...
        if (skb_info->data_off + HTTP2_FRAME_HEADER_SIZE > skb->len) {
            log_debug("http2 end of packet, %lu; %lu", skb_info->data_off, skb->len);
            break;
        }
        ...
        if (!format_http2_frame_header(&current_frame)){
            log_debug("http2 failed reading frame %lu", skb_info->data_off);
            break;
        }
...
int socket__http2_filter(struct __sk_buff *skb) {
    ...
    log_debug("http2_filter args: %lu\n", dispatcher_args_copy.skb_info.data_off);
    ...
    log_debug("http2 found %lu interesting headers in the packet", iteration_value.frames_count);
...
int socket__http2_frames_parser(struct __sk_buff *skb) {
    ...
    log_debug("http2 have a frame %lu; with flags %lu; offset %lu", current_frame.frame.type, current_frame.frame.flags, current_frame.offset);
