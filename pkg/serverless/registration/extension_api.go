...
func RegisterExtension(registrationRoute string, timeout time.Duration) (ID, error) {
	extensionRegistrationURL := BuildURL(registrationRoute)
	payload := createRegistrationPayload()

	request, err := buildRegisterRequest(headerExtName, extensionName, extensionRegistrationURL, payload)
	if err != nil {
